...
SGLANG_ARGS=(
   ...
   # `auto` lets sglang pick between intranode (NVLink p2p) and internode
   # (IB) paths per call based on the current request set.
   --sglang-moe-a2a-backend deepep
   --sglang-deepep-mode auto

   # Cap captured-CUDA-graph batch size. Default is 512 which causes
   # DeepEP's low_latency_dispatch buffer to overflow during cuda graph
   # capture (deep_ep.cpp:1105 — x.size(0) <= num_max_dispatch_tokens_per_rank).
   # Our rollout_batch_size is 64 total samples spread across 8 engines, so
   # the per-engine concurrent decode peaks well below 64. See README
   # "Crash history" for the original failure.
