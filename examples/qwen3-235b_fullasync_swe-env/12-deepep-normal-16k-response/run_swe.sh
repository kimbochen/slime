...
SGLANG_ARGS=(
   ...
   # EP=8 explicit. With --sglang-moe-a2a-backend=deepep, SGLang overrides
   # ep_size=4 → 8 anyway to match tp_size (DeepEP requires ep_size aligned
   # with the engine's TP topology). Setting it explicitly so the launcher
   # matches what SGLang actually uses.
   --sglang-ep-size 8
   ...
   # DeepEP all-to-all backend for SGLang's MoE expert dispatch on the
   # rollout side. Matches what the trainer uses
   # (--moe-flex-dispatcher-backend deepep) but for inference.
   #
   # `normal` mode (vs `auto` / `low_latency` used in experiment 11):
   # - dynamic per-call buffer allocation instead of fixed pre-allocation
   #   → no `num_max_dispatch_tokens_per_rank` constraint, no buffer-overflow
   #     crash at large cuda-graph capture batch sizes (11's first failure)
   # - doesn't reserve SMs for communication the way low_latency does
   #   (the "Only use 20 SMs for DeepEP communication" SGLang warning that
   #   bit 11 in the straggler tail)
   # - higher per-call overhead at *full* batches but lower per-call cost
   #   at *sparse* batches — better fit for our agentic-RL workload where
   #   most of each rollout is the straggler tail (1 sample on an engine)
   #
   # Keeps the DeepEP NVSHMEM/RDMA infrastructure used by the trainer's
   # weight broadcast, so update_weights_time should stay ~35s (vs 43s
   # on plain alltoall in experiment 10).
   --sglang-moe-a2a-backend deepep
   --sglang-deepep-mode normal

   # Cap captured-CUDA-graph batch size — kept from experiment 11 even
   # though `normal` mode doesn't have the same fixed-buffer constraint
   # that crashed 11's first attempt at bs=512. With our rollout_batch_size
   # of 64 spread across 8 engines, per-engine concurrent decode peaks
   # well below 64 anyway, so capping captures saves memory/startup time
