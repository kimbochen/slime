...
ROLLOUT_ARGS=(
   ...
   # Halved again from 13's 16K → 8K per turn. With the 32K trainer cap,
   # this keeps trajectories well within budget even when the model emits
   # multi-turn responses. Aggressive cap on the thinking-token blowup
   # per turn; we'll see whether the agent can still complete tasks with
   # only 8K to think per call.
...
PERF_ARGS=(
   ...
   # PP=4 + CP=2 (instead of 13's PP=2 + CP=4) brings the effective trainer
   # context cap down from 64K to 32K = CP × max_tokens_per_gpu = 2 × 16384.
   # Reusing the exact 32K-tier shape from experiment 02 (context-scaling).
   # The 8K per-turn response cap below means trajectories rarely exceed
   # 32K total, so the smaller trainer cap should still fit comfortably.
   --pipeline-model-parallel-size 4
   --context-parallel-size 2
   --expert-model-parallel-size 8
   --expert-tensor-parallel-size 1
   --sequence-parallel
   # PP=4 splits 94 layers as 24/24/24/22, so the last PP stage gets 22
   # layers (the uneven split). Required for Megatron's PP load-balancing.
...
SGLANG_ARGS=(
   ...
   # EP=8 explicit (same as 13). Inference side keeps the default SGLang
   # alltoall MoE backend — we proved in 11/12 that SGLang DeepEP doesn't
   # help on our agentic-RL workload. Trainer-side --moe-flex-dispatcher-backend
   # deepep stays on (that's Megatron's expert dispatch, unrelated to SGLang's).
   --sglang-ep-size 8

   # Cap SGLang's per-request context at 32K to match the trainer's
   # smaller effective context window (CP × max_tokens_per_gpu = 2 × 16384).
   # Halved from 13's 65536 to keep rollout and trainer caps aligned —
   # any over-cap rollout fails cleanly at the engine rather than
   # mysteriously breaking the trainer.
