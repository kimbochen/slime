...
Each tier's `.sh` knows where it sits in the tree (one extra `../` for `SLIME_ROOT`) and PYTHONPATH includes the parent example dir so `add_slime_metrics` / `coding_sandbox` / `generate_with_codingagent` are importable from a subdir.
