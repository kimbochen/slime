...
# --- JSONL metric dump (every metric, every step, in one structured file) ---
#
# The actual monkey-patch lives in lib/patch_log_utils.py and is
# auto-loaded for EVERY Python process (rollout + trainer) via the sibling
# lib/sitecustomize.py. We re-import here as a defensive fallback in case
# sitecustomize didn't run (e.g., another sitecustomize earlier on sys.path).
# patch_log_utils._patch() is idempotent — safe to call multiple times.
try:
    import patch_log_utils  # noqa: F401 — imported for the monkey-patch side effect
    patch_log_utils._patch()
except ImportError:
    pass
...
async def generate(args, sample: Sample, sampling_params) -> Sample:
    ...
    # abort+re-dispatch cycle so the agent can resume from where it left
    # off. Clearing them here would defeat partial-rollout entirely (and
    # silently — the agent's resume-detection logic checks
    # response_length > 0, which we'd nuke).
...
def custom_rollout_log_function(
    ...
    print(f"[rollout_metrics] rollout {rollout_id}: {log_dict}", flush=True)
