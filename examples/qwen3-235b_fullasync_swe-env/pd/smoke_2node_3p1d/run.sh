...
SGLANG_ARGS=(
   ...
   # 3 prefill + 1 decode, TP=4 each → 4 engines × 4 GPUs = 16 GPUs total.
   --rollout-num-gpus-per-engine 4
   --sglang-mem-fraction-static 0.85
   # DP-attention dropped here: smoke_2node_dpattn tests dp-attention with
   # 1+1 PD; this run isolates the multi-prefill-engine variable. Once both
   # pass, a future smoke would test the combination.
...
if [ -n "${WANDB_KEY}" ]; then
   WANDB_ARGS+=(
      ...
      --wandb-group qwen3-4b-2node-3p1d
      --wandb-key "${WANDB_KEY}"
   )
fi
