...
SGLANG_ARGS=(
   ...
   # Match the 12-node pd/run_swe_pd_upstream.sh SGLang config more closely:
   #  • Bump mem_fraction 0.75 -> 0.85 (matches 12-node)
   #  • Enable DP-attention (untested combo with cross-node mooncake — the
   #    smoke 30658 had it OFF because Qwen3-4B is dense and dp-attention
   #    wasn't relevant; here we're explicitly testing whether it composes
   #    with cross-node PD)
   #  • Set --sglang-dp-size 8 (matches TP=8 layout per engine)
...
if [ -n "${WANDB_KEY}" ]; then
   WANDB_ARGS+=(
      ...
      --wandb-group qwen3-4b-2node-dpattn
      --wandb-key "${WANDB_KEY}"
   )
fi
...
{
  "env_vars": {
    "PYTHONPATH": "${MEGATRON_LM_PATH}:${SCRIPT_DIR}:${SLIME_ROOT}",
