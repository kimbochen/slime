...
async def _read_stream_capped(stream: Any, max_bytes: int) -> str:
    ...
    Modal forces sandbox streams into TEXT mode and uses a strict UTF-8
    incremental decoder internally (modal/io_streams.py:_decode_bytes_stream_to_str
    creates the decoder with errors="strict" and we have no way to override
    it). If a command emits a non-UTF-8 byte (e.g. `cat` on a binary file,
    `git diff` on a binary blob, or a buggy patch producing mojibake), the
    decoder raises UnicodeDecodeError and the async iterator dies. We catch
    that here, return whatever we collected up to the bad byte, and append
    a sentinel so the caller (and the agent) see the truncation rather than
    losing the entire trajectory.
    """
    buf = bytearray()
    remaining = max_bytes
    try:
        ...
        buf.extend(b"\n<truncated: command produced non-utf8 output>\n")
