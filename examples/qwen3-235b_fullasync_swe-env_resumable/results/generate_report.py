...
def parse_rollout_lines(log_path: Path) -> list[dict]:
    pattern = re.compile(r"add_slime_metrics\] rollout (\d+): (\{.*\})")
    rollouts = []
    for line in log_path.read_text(errors="replace").splitlines():
        m = pattern.search(line)
        if not m:
...
def render_report(
    ...
    lines: list[str] = []
    lines.append(f"# Resumable SWE-env run — job {job_id}")
    lines.append("")
...
def main():
    if len(sys.argv) != 3:
        print(f"Usage: {sys.argv[0]} <slurm.out> <output_dir>", file=sys.stderr)
        sys.exit(1)
    log_path = Path(sys.argv[1])
    out_dir = Path(sys.argv[2])
    out_dir.mkdir(parents=True, exist_ok=True)

    # Try to detect job_id from log filename like infx-swe-rsm-30678.out
    ...
    # Render markdown
    md = render_report(steps, rollouts, rewards, job_id, log_path, out_dir / "RESULTS.md")
    print(md)
