...
Hermetic tests for the metadata-based metric fix in add_slime_metrics.py.
...
Run:
    uv run python -m pytest examples/qwen3-235b_fullasync_swe-env_resumable/test_metric_fix.py -v

(or just `uv run python examples/qwen3-235b_fullasync_swe-env_resumable/test_metric_fix.py`
...
# ---------------------------------------------------------------------------
# Step 1 — stub the heavy modules before importing add_slime_metrics. Each
# test gets a fresh, isolated `add_slime_metrics` module instance via
...
def _install_stubs():
    """Install minimal stubs of the modules add_slime_metrics imports at top.

    Returns the stubbed `generate_with_codingagent` module so tests can swap
    in a fake `generate` coroutine when needed.
    """
    # Wipe any prior copies so each test starts clean.
    for k in list(sys.modules):
        if k == "add_slime_metrics" or k.startswith("slime.") or k in (
    ...
    logged: list[dict] = []
    logging_mod.log = lambda args, d, **k: logged.append(d)
    ...
    # add_slime_metrics monkey-patches)
...
def _load_module_from(example_dir: str):
    ...
    add_slime_metrics module instance + helpers."""
    Sample, gca, logging_mod = _install_stubs()
    if example_dir in sys.path:
        sys.path.remove(example_dir)
    sys.path.insert(0, example_dir)
    A = importlib.import_module("add_slime_metrics")
...
EXAMPLES = [
    "/home/sa-shared/kimbo/slime/examples/qwen3-235b_fullasync_swe-env",
    "/home/sa-shared/kimbo/slime/examples/qwen3-235b_fullasync_swe-env_resumable",
]
...
def test_partial_rollout_does_not_wipe_state(example_dir):
    ...
    etc. — otherwise resume detection in the agent fails silently."""
    A, Sample, gca, _ = _load_module_from(example_dir)
    s = Sample(
        tokens=[1, 2, 3],
        response_length=3,
...
def test_no_partial_rollout_wipes_state(example_dir):
    """Without partial_rollout, the wipe is intentional (matches retool)."""
    A, Sample, gca, _ = _load_module_from(example_dir)
    s = Sample(
        tokens=[1, 2, 3],
        response_length=3,
